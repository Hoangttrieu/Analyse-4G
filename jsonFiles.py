...
def processingJson(pathnamefile,coordinates,namefile):
    ...
    for i in range (0,len(json_objects)):
       ...
       # frame_object["frame.comment"] = coordinates[i]
        frame_object["frame.comment"] = {"frame.comment.geolocation":coordinates[i]}
    ...
    for index,row in df.iterrows():
        ...
        for i in range(0,len(df1.columns),20):
            if df1.iloc[index][71+i+4] is not None:
                neighborCount+=1
                neighborList["Neighbor "+str(neighborCount)]={"PCI": df1.iloc[index][71 + i + 4], "RSRP": df1.iloc[index][71 + i + 6],"RSRQ": df1.iloc[index][71 + i + 7], "RSSI": df1.iloc[index][71 + i + 8]}

        #Prepare the json file
        json_obj["current cell"]={}
        json_obj["current cell"]["Date"]=row["Date"]
        time=str(row["Time"].strftime('%H:%M:%S.%f'))
        json_obj["current cell"]["Time"] =time
        ...
        json_obj["Neighbors cells"]["Neighbors.information"]=neighborList
